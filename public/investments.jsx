...
/* ============================================================
   COST-BASIS RETURN — the user's ACTUAL unrealized return.
   Sums what was paid (shares x avg cost) over non-cash holdings
   that actually have a cost, and compares it to current value.
   Holdings with no cost are skipped from the cost side (their
   value would otherwise distort the % vs an unknown basis).
   A position bought today (cost ~ current price) reads ~0%,
   unlike the backfilled portfolio series which shows the
   security's whole-period market move as if held all along.
   ============================================================ */
function invCostBasisReturn(holdings) {
  let cost = 0, value = 0;
  ...
  return { cost, value, gain, pct, hasCost: cost > 0 };
}
...
function InvestmentsPage({ holdings, onOpen, onEdit, onDelete }) {
  ...
  // Headline "Total return" reflects the user's ACTUAL unrealized gain vs what
  // they paid (cost basis) — NOT the backfilled current-shares-at-old-prices
  // series, which over-states return for anything bought mid-period.
  const invCb = invCostBasisReturn(holdings);
  const invTotalRet = invCb.pct;       // % gain vs cost basis (0% for a same-day buy)
  const invTotalRetDollar = invCb.gain; // unrealized $ gain
  // Benchmark return still framed over the selected period (market context).
  const yrSp = spRet;
    ...
    <React.Fragment>
      ...
      <div className="kpi-row">
        ...
        <Card widget><div className="kpi">
          ...
          <span className={"kpi-val " + (invTotalRet >= 0 ? "pos" : "neg")}>{invCb.hasCost ? invPct(invTotalRet) : "\u2014"}</span>
          <span className={"kpi-delta " + (invTotalRetDollar >= 0 ? "pos" : "neg")}>{invCb.hasCost ? (invTotalRetDollar >= 0 ? "\u2191" : "\u2193") + " " + invSigned(invTotalRetDollar, 0) + " unrealized" : "add avg cost to track"}</span>
        </div></Card>
        <Card widget><div className="kpi">
        ...
        <Card widget><div className="kpi">
          <span className="kpi-label">Holdings vs S&amp;P 500 · {period}</span>
          <span className="kpi-val" style={{ color: gap >= 0 ? "var(--green)" : "var(--red)" }}>{invPct(gap)}</span>
          <span className="kpi-delta" style={{ color: "var(--muted)" }}>{gap >= 0 ? "ahead of the market" : "behind the market"}</span>
        </div></Card>
      </div>
      ...
      <Card widget>
        <div className="cf-hero-head">
          <div>
            <span className="widget-eyebrow">Market performance of current holdings</span>
            <div className="cf-hero-title">Your holdings vs the S&amp;P 500</div>
          </div>
        ...
        <div className="cmp-readout">
          <span className="cmp-chip"><span className="cmp-dot" style={{ background: "var(--accent)" }} />Holdings <b style={{ color: pfRet >= 0 ? "var(--green)" : "var(--red)" }}>{invPct(pfRet)}</b></span>
          <span className="cmp-chip"><span className="cmp-dot" style={{ background: "var(--muted)" }} />S&amp;P 500 <b>{invPct(spRet)}</b></span>
          {invCb.hasCost &&
            <span className="cmp-chip" title="Your actual return vs what you paid (cost basis)"><span className="cmp-dot" style={{ background: "var(--green)" }} />Your return <b style={{ color: invTotalRet >= 0 ? "var(--green)" : "var(--red)" }}>{invPct(invTotalRet)}</b></span>}
