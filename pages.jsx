...
const PG_BUDGET_STORE = window.__claudBudgetStore || (window.__claudBudgetStore = (() => {
  ...
  return {
    ...
    set: (g) => {
      groups = g;
      subs.forEach((fn) => { try { fn(g); } catch (e) {} });
      window.dispatchEvent(new CustomEvent("claud:budgets-changed"));
    },
...
// month elapsed: June 13 → 13/30
const PG_MONTH_ELAPSED = 13 / 30;
...
function BudgetPage({ coverStyle = "suggested" }) {
  ...
  // seed from the shared store if it already holds edits (e.g. user came back to this tab),
  // otherwise from the static groups. Keep the store in sync on every change so Foresight
  // can read the live budgets even while this page is unmounted.
  const [groups, setGroups] = pgUseState(() => PG_BUDGET_STORE.get() || pgSeedGroups());
  React.useEffect(() => { PG_BUDGET_STORE.set(groups); }, [groups]);
  ...
  React.useEffect(() => {
    const open = () => setModal({ mode: "add", group: groups[0] ? groups[0].label : "" });
  ...
  const pacePct = Math.round(PG_MONTH_ELAPSED * 100);
  ...
  function saveCat(data) {
    // data: { id?, name, group, budget, color }
    setGroups((prev) => {
      // remove any existing instance of this id
      let next = prev.map((g) => ({ ...g, cats: g.cats.filter((c) => c.id !== data.id) }));
      const entry = {
        id: data.id || "new-" + Date.now(),
        name: data.name, budget: data.budget, color: data.color,
        spent: data.spent != null ? data.spent : 0,
        roll: data.roll != null ? data.roll : 0
      };
      next = next.map((g) => g.label === data.group ? { ...g, cats: [...g.cats, entry] } : g);
      return next;
    });
    setModal(null);
  }
  function removeCat(id) {
    setGroups((prev) => prev.map((g) => ({ ...g, cats: g.cats.filter((c) => c.id !== id) })));
  }
  // move category `fromId` to occupy `toId`'s slot, within one group
  ...
  function openCover(c) {
    ...
    setCover({ id: c.id, name: c.name, spent: c.spent, color: c.color, avail });
  }
  ...
  function applyCover(moves) {
    const targetId = cover.id;
    const total = moves.reduce((s, m) => s + m.amt, 0);
    setGroups((prev) => prev.map((g) => ({
      ...g, cats: g.cats.map((c) => {
        if (c.id === targetId) return { ...c, budget: c.budget + total };
        const mv = moves.find((m) => m.id === c.id);
        if (mv) return { ...c, budget: c.budget - mv.amt };
        return c;
      })
    })));
    setCover(null);
    ...
    <React.Fragment>
      ...
      <div className="kpi-row">
        <Card widget><div className="kpi">
          <span className="kpi-label">Budgeted · June</span>
      ...
      <Card widget>
        <div className="cf-hero-head">
          <div>
            <span className="widget-eyebrow">June budget</span>
        ...
        return (
          <Card widget key={g.label}>
            ...
            <div className="cat-list">
                    ...
                    <div className="cat-top">
                      ...
                      <span className="cat-right">
                        ...
                        <span className="cat-actions">
                          <button className="cat-act" title="Edit category" aria-label={"Edit " + c.name} onClick={() => setModal({ mode: "edit", cat: c, group: g.label })}><Icon name="pencil" /></button>
              ...
              <button className="cat-add" onClick={() => setModal({ mode: "add", group: g.label })}>
...
function BudgetModal({ modal, groups, onClose, onSave, onDelete }) {
  ...
  const [group, setGroup] = pgUseState(modal.group || (groups[0] && groups[0].label) || "");
