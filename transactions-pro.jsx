...
/* rules: [{ id, match (merchant substring), cat }] */
const TXP_RULES = window.__claudTxnRules || (window.__claudTxnRules = (() => {
  ...
  s.add = (rule) => {
    ...
    if (!match) return;
    const rest = s.get().filter((r) => r.match.toLowerCase() !== match.toLowerCase());
    s.set([{ id: "r" + Date.now(), match, cat: rule.cat }, ...rest]);
  };
  s.remove = (id) => s.set(s.get().filter((r) => r.id !== id));
  s.update = (id, patch) => s.set(s.get().map((r) => r.id === id ? { ...r, ...patch } : r));
  return s;
})());
...
function useTxnRules() {
  const [r, set] = React.useState(TXP_RULES.get());
  React.useEffect(() => TXP_RULES.subscribe(set), []);
  return r;
}
/* first rule whose merchant-substring is contained in the transaction name */
function txpRuleFor(name, rules) {
  const n = (name || "").toLowerCase();
  return rules.find((r) => r.match && n.includes(r.match.toLowerCase())) || null;
}
...
function TxnDetailDrawer({ txn, onClose, onRecat, onRemove, onSetIcon, onViewReceipt, hasRule }) {
  ...
  const note = m.note || "";
  const attachment = m.attachment || txn.receipt || null;
  const attachName = m.attachName || (txn.receipt ? "Receipt photo" : null);
